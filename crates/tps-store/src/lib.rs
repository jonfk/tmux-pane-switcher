...
impl Store {
    ...
    pub fn upsert_snapshots(&mut self, snapshots: &[PaneSnapshot]) -> Result<usize> {
        ...
        tx.execute(
            "INSERT INTO servers(server_key, created_at, last_seen_at)
             VALUES (?1, ?2, ?2)
             ON CONFLICT(server_key) DO UPDATE SET last_seen_at = excluded.last_seen_at",
            params![server_key, observed_at],
        )?;
    ...
    pub fn list_ranked(&self, server_key: &str, limit: usize) -> Result<Vec<RankedPane>> {
        ...
        let rows = statement.query_map(params![server_key, limit], |row| {
            Ok(RankedPane {
                ...
                window_index: row.get(5)?,
                window_name: row.get(6)?,
                pane_index: row.get(7)?,
                display_title: row.get(8)?,
                cwd: row.get(9)?,
                current_command: row.get(10)?,
                process_class: parse_process_class(row.get::<_, String>(11)?),
                is_running: sqlite_to_bool(row.get::<_, i64>(12)?),
                is_interesting: sqlite_to_bool(row.get::<_, i64>(13)?),
                window_active: sqlite_to_bool(row.get::<_, i64>(14)?),
                pane_active: sqlite_to_bool(row.get::<_, i64>(15)?),
                last_activity_at: row.get(16)?,
                rank_updated_at: row.get(17)?,
            })
        })?;
...
mod tests {
    ...
    fn ranking_prefers_recent_window_activity() {
        ...
        let ranked = store.list_ranked("/tmp/tmux.sock", 2).expect("rank panes");
    ...
    fn snapshot(
        ...
        PaneSnapshot {
            server_key: "/tmp/tmux.sock".to_string(),
    ...
    fn temp_db_path() -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("tmux-pane-switcher-{unique}.sqlite"))
    }
}
